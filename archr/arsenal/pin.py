...
class PINTracerBow(ContextBow):
    ...
    def fire_context(self, args_prefix=None, basic_blocks=True, calls=False, syscalls=False, branches_only=False, main_object_only=False, **kwargs): #pylint:disable=arguments-differ
        """
        Starts PIN with a fresh process.

        :param args_prefix: Additional prefix arguments to run_command
        :param basic_blocks: Trace BBL addresses
        :param calls: Trace Calls
        :param syscalls: Trace System Calls
        :param branches_only: Trace branches only
        ...
        args_prefix += ['-bbl'] if basic_blocks else []
        args_prefix += ['-calls'] if calls else []
        args_prefix += ['-sys'] if syscalls else []
        args_prefix += ['-branches'] if branches_only else []
        args_prefix += ['-main'] if main_object_only else []
        args_prefix += ['--']
